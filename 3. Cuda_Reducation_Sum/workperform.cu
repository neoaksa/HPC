...
__global__ void cu_vector_dot (int *output_d, unsigned int vecSize)
{
	// genrate vector for F dot produt D
	__shared__ unsigned int vec[block_Size];
	...
	// when tid<=32, it's in a warp, we don't need syncthreads
	if (tid < 32) {
...
extern "C" void innerproduct(int *output,  int vecSize, int blockSize)
{
	...
	result = cudaMalloc ((void**) &output_d, sizeof(int) * blockNum);
	...
	result = cudaMemcpy (output_d, output, sizeof(int) * blockNum, cudaMemcpyHostToDevice);
	if (result != cudaSuccess) {
	...
	result = cudaMemcpy (output, output_d, sizeof(int) * blockNum, cudaMemcpyDeviceToHost);
	if (result != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy host <- dev (block) failed.");
		exit(1);
	}
