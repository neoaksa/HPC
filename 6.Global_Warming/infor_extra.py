...
def parse_record_detail(record):
    ...
    wind_speed = int(record[65:69])
    air_temp = int(record[87:92])
    return (date[0:4],latitude,longitude,wind_speed,air_temp)
...
# rdd = sc.textFile("./{2000}/*.gz")
rdd = sc.textFile("/home/DATA/NOAA_weather/{198[0-9]}/*.gz")
# rdd = sc.textFile("/home/DATA/NOAA_weather/{2000}/*.gz")
...
if is_agg_year:
    ...
    df = sqlContext.createDataFrame(all_fields,schema=["date","lat","lon","wind_speed","temp"])
    df = df.where((df['lat']!='+9999') & (df['lon']!='+9999') & (df['wind_speed']!=9999) & (df['temp']!=9999))
    df.groupBy(['date',"lat","lon"]).agg({"wind_speed":"avg","temp":"avg"}).coalesce(1).write.csv(output1_name)



# ~/spark/sbin/start-all.sh
# ~/spark/sbin/stop-all.sh
# ~/spark/bin/spark-submit --master=spark://dc11.cis.gvsu.edu:7077 infor_extra.py
